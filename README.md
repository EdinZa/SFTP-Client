...
The SAM CLI installs dependencies defined in `Upload/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
SFTP-Client$ sam local invoke Upload Function --event events/event.json
```
...
```bash
SFTP-Client$ sam logs -n Upload Function --stack-name SFTP-Client --tail
```
